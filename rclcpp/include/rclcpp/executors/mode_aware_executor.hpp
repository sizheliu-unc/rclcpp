...
public:
  ...
  /// Perform mode change following Section 6.2 protocol.
  /**
   * Classifies each callback per the protocol:
  ...
  void mode_update(const StateT & state)
  {
    ...
    // Reject concurrent mode changes
    if (transition_in_progress_.load()) {
      const auto logger = rclcpp::get_logger("ModeAwareExecutor");
      RCLCPP_WARN(logger, "Mode transition already in progress; ignoring MCR");
      return;
    }

    // Join previous transition thread if it completed
    if (transition_thread_.joinable()) {
      transition_thread_.join();
    }
    ...
    if (offset_it != mode_offsets_.end()) {
      offsets = offset_it->second;
    }
    ...
    struct DeferredUpgrade {
      ...
      rclcpp::sched::SchedAttr new_attr;
      int64_t offset_ns;
    };
    ...
    for (const auto & [cb_name, entity] : named.entities_by_name) {
      ...
      if (in_old && !in_new) {
        ...
        apply_sched_attr_to_entity(entity, make_other_attr());

      } else if (!in_old && in_new) {
        // (b) WHOLLY NEW: introduced after offset Y_i
        if (offset_ns > 0) {
          RCLCPP_INFO(logger, "  [WHOLLY NEW] '%s' -> SCHED_OTHER now, FIFO(%u) after %ld ns",
            cb_name.c_str(), new_prio, offset_ns);
          apply_sched_attr_to_entity(entity, make_other_attr());
          deferred.push_back({entity, make_fifo_attr(new_prio), offset_ns});
        } else {
          RCLCPP_INFO(logger, "  [WHOLLY NEW] '%s' -> FIFO(%u) immediately", cb_name.c_str(), new_prio);
          apply_sched_attr_to_entity(entity, make_fifo_attr(new_prio));
        }

      } else if (in_old && in_new && old_prio != new_prio) {
        // (c) CHANGED: keep old priority during offset, then apply new
        if (offset_ns > 0) {
          RCLCPP_INFO(logger, "  [CHANGED] '%s' FIFO(%u) -> FIFO(%u) after %ld ns",
            cb_name.c_str(), old_prio, new_prio, offset_ns);
          // Keep old priority (no change now)
          deferred.push_back({entity, make_fifo_attr(new_prio), offset_ns});
        } else {
          RCLCPP_INFO(logger, "  [CHANGED] '%s' FIFO(%u) -> FIFO(%u) immediately",
            cb_name.c_str(), old_prio, new_prio);
          apply_sched_attr_to_entity(entity, make_fifo_attr(new_prio));
        }

      } else if (in_old && in_new && old_prio == new_prio) {
        // (d) UNCHANGED: continue uninterrupted, or with Z_i offset
        if (offset_ns > 0) {
          RCLCPP_INFO(logger, "  [UNCHANGED+Z] '%s' FIFO(%u) Z offset %ld ns",
            cb_name.c_str(), old_prio, offset_ns);
          // Keep current priority; re-apply at offset for any potential threadgroup changes
          deferred.push_back({entity, make_fifo_attr(new_prio), offset_ns});
        } else {
    ...
    // Step 7: Spawn transition thread for deferred upgrades
    if (!deferred.empty()) {
      ...
      {
        // Set this thread to SCHED_FIFO priority 99 (maximum)
        ...
        for (const auto & [offset_ns, indices] : offset_groups) {
          ...
          // Sleep until the absolute wakeup time
          int err;
          do {
          ...
          // Apply deferred priority upgrades for this offset group
          for (size_t idx : indices) {
            const auto & upgrade = deferred[idx];
            if (upgrade.entity) {
              apply_sched_attr_to_entity(upgrade.entity, upgrade.new_attr);
            }
          }
